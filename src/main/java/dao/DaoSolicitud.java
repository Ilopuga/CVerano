...
/**
 * Clase DAO para manejar las operaciones de la base de datos relacionadas con
 * las entidades de tipo {@link Solicitud} Implementa el patrón Singleton para
 * asegurar una única instancia de la conexión a la base de datos.
 * 
...
public class DaoSolicitud {
	 ...
	 * 
	 * @throws SQLException Si no puede establecer una conexión con la base de
	 *                      datos.
	 */
	...
	/**
	 * Obtiene una solicitud por DNI. Este método es utilizado principalmente para
	 * administración y gestión de solicitudes individuales.
	 * 
	...
	private ArrayList<Solicitud> listar() throws SQLException {
		...
		while (rs.next()) {
			...
			result.add(new Solicitud(rs.getInt("id"), rs.getString("Dni"), rs.getInt("cod_actividad"),
					...
					rs.getInt("num_sorteo"), rs.getBoolean("seleccionado"), rs.getBoolean("pago"),
					rs.getString("estado"), rs.getInt("edicion")));
	 ...
	 * @throws SQLException Si ocurre un error durante la consulta o la
	...
	/**
	 * Elimina una solicitud de la base de datos utilizando su ID como
	 * identificador.
	 * 
	...
	/**
	 * Verifica si existe una solicitud con el DNI dado. Este método consulta para
	 * determinar si hay alguna solicitud asociada con el DNI dado.
	 * 
	...
	/**
	 * Asigna números de sorteo secuenciales a todas las solicitudes, ordenadas por
	 * ID. Este método es utilizado para gestionar el proceso de selección basado en
	 * sorteos.
	 * 
	 * @throws SQLException Si ocurre un error durante la actualización de los
	 *                      datos.
	 */
	// Una vez controlados por el admin las solictudes recibidas, para un proceso
	// justo
