...
public class GestionSolicitud extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (opcion == 1) {
			try {
				...
				if (dao.dniExiste(dni)) {
					...
					Solicitud s = new Solicitud(dni, cod_actividad, nombre, apellido1, apellido2, email, direccion,
		...
		if (opcion == 2) {
			...
			try {
				Solicitud s = new Solicitud(dni, cod_actividad, nombre, apellido1, apellido2, email, direccion,
						telefono, f_nacimiento, num_sorteo, seleccionado, pago, estado, edicion);
				int idInt = id;
				s.setId(idInt);
				s.actualizar();
